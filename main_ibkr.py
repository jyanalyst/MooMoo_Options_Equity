#!/usr/bin/env python3
"""
Options Scanner - Main Entry Point (IBKR variant)
Scans for Wheel Strategy candidates using Interactive Brokers for options data.

This is the IBKR counterpart to main.py. Stock quotes still come from FMP and
historical prices from yfinance; only the options chain + Greeks are sourced
from IBKR (via ib_async / IB Gateway). See ibkr_data_fetcher.py.

Usage:
    python main_ibkr.py wheel              # Run Wheel screener
...
|            Options data via IBKR API (IB Gateway)             |
|                                                               |
+===============================================================+
    """)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Options Income Scanner - Wheel Strategy (IBKR)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python main_ibkr.py wheel                          # Scan full universe (~40 stocks)
  python main_ibkr.py wheel --liquid-only            # Scan liquid names only (~15 stocks, faster)
  python main_ibkr.py wheel --capital 6700           # Scan stocks requiring <=$6,700/position
  python main_ibkr.py wheel --liquid-only --capital 20000  # Liquid names under $20k
  python main_ibkr.py --mock wheel                   # Test with mock data
  python main_ibkr.py wheel --no-csv                 # Scan without CSV export
        """,
    )
    ...
    parser.add_argument(
        "--mock",
        action="store_true",
        help="Use mock data (for testing without IBKR connection)",
    )
    ...
    # Check availability
    if not args.mock:
        from ibkr_data_fetcher import YFINANCE_AVAILABLE

        if not YFINANCE_AVAILABLE:
            print("[ERROR] yfinance not available. Install with: pip install yfinance")
            sys.exit(1)
        if not IB_ASYNC_AVAILABLE:
            print("[WARN] ib_async not available - options features will not work")
    ...
    try:
        fetcher = get_ibkr_data_fetcher(use_mock=args.mock)

        if not args.mock:
            # Connect to IB Gateway for options data
            if not fetcher.connect():
                print("\n[WARN] Could not connect to IB Gateway.")
                print("       Options data will not be available.")
                print(
                    "       Make sure IB Gateway/TWS is running, logged in, and the API"
                )
                print(
                    "       port matches config.IBKR_PORT (default 4001 = Gateway live)."
                )
