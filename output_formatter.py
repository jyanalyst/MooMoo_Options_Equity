...
class OutputFormatter:
    ...
    def __init__(self, output_dir: str = None):
        ...
        """
        self.output_dir = output_dir or OUTPUT_CONFIG.get(
    ...
    def display_wheel_results(self, candidates: List[Dict], show_all: bool = False):
        ...
        print(f"\n{'=' * 70}")
        print(
            f">> WHEEL STRATEGY CANDIDATES - {datetime.now().strftime('%Y-%m-%d %H:%M')}"
        )
        print(f"{'=' * 70}")
        ...
        # Header
        print(
            f"{'Rank':<5} {'Ticker':<8} {'Price':>8} {'Exp':<12} {'DTE':>4} "
        ...
        for i, c in enumerate(display_list, 1):
            ...
            ivr_str = f"{iv_rank:>4.0f}%" if iv_rank is not None else f"{'N/A':>5}"
            print(
                f"{i:<5} {c['ticker']:<8} ${c['price']:>6.2f} {c['expiration']:<12} "
                f"{c.get('dte', 0):>4} ${opt.get('strike', 0):>6.2f} "
                f"{opt.get('delta', 0):>5.2f} ${opt.get('premium', 0):>5.2f} "
                f"{opt.get('return_pct', 0):>5.1f}% {ivr_str} "
    ...
    def display_detailed_candidate(self, candidate: Dict, strategy: str = "wheel"):
        ...
        print(
            f"IV Rank:       {c.get('iv_rank', 'N/A')}% ({c.get('iv_method', 'N/A')})"
        )
        ...
        print(
            f"  Return:       {opt.get('return_pct', 0):.2f}% "
    ...
    def print_scan_summary(self, candidates: List[Dict] = None):
        ...
        if candidates is not None:
            ...
            if candidates:
                top = candidates[0]
                print(
                    f"   Top candidate: {top['ticker']} (Score: {top.get('quality_score', 0):.1f})"
                )
